...
Orbit::Orbit(float orbitRadius, const glm::vec3& orbitColor) 
    : radius(orbitRadius), color(orbitColor), vertexCount(SEGMENTS) {
    setupOrbit();
}
...
void Orbit::setupOrbit() {
    ...
    glBufferData(GL_ARRAY_BUFFER, SEGMENTS * 3 * sizeof(float), vertices, GL_STATIC_DRAW);
...
void Orbit::render(Shader& shader, const glm::mat4& view, const glm::mat4& projection) {
    shader.use();
